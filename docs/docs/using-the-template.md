...
Right now on GitHub, it is hard to observe and comment on changes that happen in Jupyter notebooks. We develop and maintain a tool called [`nbautoexport`](https://nbautoexport.drivendata.org/stable/) that automatically exports a `.py` version of your Jupyter notebook every time you save it. This means that you can commit both the `.ipynb` and the `.py` to source control so that reviewers can leave line-by-line comments on your notebook code. 

You have the option to automatically install the package when creating your own cookie cutter repo through the `notebook_export` option. Select `nbautoexport`. Once the repo is created, run `make nbautoexport` to configure the notebooks directory. 

Alteratively, you can import the package yourself by adding `nbautoexport` to your requirements file and then run `make requirements` to install it. Once `nbautoexport` is installed, you can setup the nbautoexport tool for your project manually with the following commands at the commandline:
